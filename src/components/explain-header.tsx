...
import { parseHeader, enhanceExplanation, type CacheControlDirective } from '@/lib/cache-control';
      ...
      <Show when={props.headerValue.trim() !== ''}>
        ...
        <div class="space-y-4">
          <Show when={parsedDirectives().length === 0}>
            <p class="text-muted-foreground">No valid directives found in the header.</p>
          </Show>
...
const SummarySection: Component<SummarySectionProps> = (props) => {
  ...
  const summary = createMemo(() => {
    ...
    const hasMustRevalidate = directives.some(d => d.name === 'must-revalidate');
  ...
  return (
    <div class="pt-2">
      ...
      <Card class="p-4 mt-2 bg-muted/30">
        <p>{summary()}</p>
      </Card>
    </div>
  );

  ...
  'relative w-full rounded-lg border px-4 py-3 text-sm [&:has(svg)]:pl-11 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground',
  {
    variants: {
      variant: {
        ...
        warning:
          'border-yellow-500/50 text-yellow-600 dark:border-yellow-500 dark:text-yellow-500 bg-yellow-50 dark:bg-yellow-950/20 [&>svg]:text-yellow-600 dark:[&>svg]:text-yellow-500',
      },
    },
    defaultVariants: {
      variant: 'default',
...
export const AlertTitle = (props: ComponentProps<'div'>) => {
  ...
  return (
    <div
      class={cn('leading-5 font-medium tracking-tight', local.class)}
      {...rest}
    />
  );
};
